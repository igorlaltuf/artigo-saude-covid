...
while (i<=length(lista.de.arquivos)) {
  ...
  # Classificação segundo a Tabela de Conversão da Classificação Brasileira de Ocupação 
  ...
  anos2007.2010 <- x %>%
    ...
    group_by(codufmun, ano) %>% # usar tpgestao aqui para saber os médicos do município, estado ou duplo
    summarise(qtd_med_sus = sum(prof_sus)) %>%
    left_join(pop, by = c('ano', 'codufmun' = 'cod_muni')) %>%    # continuar daqui!
    mutate(med_sus_100_mil_hab = (qtd_med_sus/populacao)*100000)
